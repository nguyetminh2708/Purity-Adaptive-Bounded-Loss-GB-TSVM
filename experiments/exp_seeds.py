"""Multi-seed robustness run (fixed params) for hard / wave / wave2.

Each seed re-draws the k-fold split, the injected noise, and the KMeans ball
splitting together. Fixed hyper-params (no inner CV) so we can afford many seeds;
the point is the seed-to-seed spread, not per-dataset tuning.

Output long CSV: dataset,kind,rate,seed,model,acc  (acc = mean over 5 folds).

    python experiments/exp_seeds.py [--seeds 5] [--kernel linear]
"""
...
def _wave(balls, F, y, steps):
    if not viable(balls): return np.nan
    C, r, yb, p, n = arrays(balls)
    try: m = WaveGBTSVM(steps=steps, **WAVE).fit(C, r, yb, purity=p, size=n)
...
def cell(name, kind, rate, seed, kernel, steps):
    ...
    for tr, te in StratifiedKFold(5, shuffle=True, random_state=seed).split(X, y):
        ...
        ah.append(_hard(bp, Fte, yte)); aw.append(_wave(bp, Fte, yte, steps))
    return np.nanmean(ah), np.nanmean(aw)
...
def main():
    ...
    out = str(HERE.parent / "results" / f"_partial_seeds_{args.kernel}.csv")
    rows = []
    t0 = time.time()
    for name in args.datasets:
        for kind, rate in combos:
            for seed in range(args.seeds):
                try:
                    h, w = cell(name, kind, rate, seed, args.kernel, args.steps)
                except Exception as e:
                    print(f"  skip {name}/{kind}/{rate}/s{seed}: {type(e).__name__}", flush=True); continue
                for m, v in (("hard", h), ("wave", w)):
    ...
    fn = save_run(pd.DataFrame(rows), f"seeds_{args.kernel}",
                  dict(kernel=args.kernel, seeds=args.seeds, steps=args.steps,
                       params="hard d=.1; wave lam0=1 fixed; wave2 binom eta=rate"))
    print(f"saved {fn}")
