"""Quét ngưỡng RESCUE floor: lớp có < floor bóng thì bơm thêm tâm-thiểu-số.
floor=1 = chỉ cứu khi lớp=0 (bản hiện tại). floor cao = cứu cả lớp 'gần cạn'.

Cấu hình (Wave λ=1 kappa=0, pur=--purity, linear):
  none  : không rescue (đè đa số)
  f1,f5,f10,f20 : rescue với floor = 1/5/10/20

    python experiments/exp_floor.py [--purity 0.7] [--seeds 3]
"""
...
def cell(name, kind, rate, seed, kernel, steps, pur):
    ...
    out = {"none": []} | {f"f{fl}": [] for fl in FLOORS}
    for tr, te in StratifiedKFold(5, shuffle=True, random_state=seed).split(X, y):
        ...
        C0, r0, y0, _, _ = arrays(b)
        out["none"].append(_wave(C0, r0, y0, Fte, yte, steps) if viable(b) else np.nan)
        for fl in FLOORS:
            Cf, rf, yf = rescue_arrays(b, fl)
            out[f"f{fl}"].append(_wave(Cf, rf, yf, Fte, yte, steps))
...
def main():
    ...
    CFG = ["none"] + [f"f{fl}" for fl in FLOORS]
    out = str(HERE.parent / "results" / f"_partial_floor_{args.kernel}.csv")
    rows = []; t0 = time.time()
    print(f"pur={args.purity} floors={FLOORS} seeds={args.seeds}", flush=True)
    ...
    fn = save_run(df, f"floor_{args.kernel}",
                  dict(kernel=args.kernel, seeds=args.seeds, purity=args.purity, floors=str(FLOORS)))
    # tóm tắt: acc theo (kind,rate,config) + overall
